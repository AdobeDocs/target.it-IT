...
* Utilizza mboxTrace o mboxDebug per risolvere i problemi della richiesta [!DNL Target].
* Utilizza Adobe Experience Cloud Debugger, uno strumento di facile utilizzo che fornisce molte delle stesse informazioni di mboxDebug, per risolvere i problemi della richiesta [!DNL Target].

mboxDebug è particolarmente utile quando imposti [!DNL Target] sulla pagina per assicurarti che la richiesta [!DNL Target] sia in esecuzione e che il cookie sia impostato. Tuttavia, non fornisce il livello di dettaglio necessario durante il debug della distribuzione dei contenuti. Se l’attività non viene visualizzata nella pagina o un contenuto indesiderato viene visualizzato, esamina ed esegui il debug della pagina nel dettaglio tramite mboxTrace.

## Recupera il token di autorizzazione da utilizzare con gli strumenti di debug {#section_BED130298E794D1FA229DB7C3358BA54}
...
mboxTrace consente di ricevere informazioni di traccia collegate alle risposte [!DNL Target]. Le informazioni di traccia riflettono il risultato di una chiamata [!DNL Target] (ad esempio, una conversione o un’impression) ed eventuali dati aggiuntivi che possono aiutare a determinare perché si è verificato questo particolare risultato, ad esempio un set di rami disponibili tra i quali è stata effettuata la selezione in una campagna. Esegui il debug della distribuzione dei contenuti con queste informazioni.
...
**Utilizzo di mboxTrace nelle pagine** dei consigli: L’aggiunta di mboxTrace come parametro di query nelle pagine con i consigli sostituisce la progettazione di Recommendations nella pagina con una finestra dei dettagli mboxTrace, che visualizza informazioni approfondite sui consigli, tra cui:
...
|--- |--- |
| `mboxDebug=1` | Debugger<br>L’aggiunta di questo parametro a qualsiasi URL con richieste Target definite consente di aprire una finestra a comparsa con dettagli utili sul debug. Vengono inserite informazioni sui cookie, valori PCid e ID di sessione e sono visibili tutti gli URL Fai clic su un URL di richiesta Target per visualizzare la risposta per la richiesta [!DNL Target]. Maggiori dettagli sono disponibili in [mbox_debug.pdf](/help/assets/mbox_debug.pdf). |
...
Il debugger di Adobe Experience Cloud facilita e velocizza la comprensione dell&#39;implementazione di Target. Puoi visualizzare rapidamente la configurazione della libreria, esaminare le richieste per verificare che i parametri personalizzati vengano passati correttamente, attivare la registrazione della console e disattivare tutte le richieste Target. Esegui l’autenticazione nell’Experience Cloud e puoi utilizzare il potente strumento MboxTrace per controllare le tue attività, le qualifiche del pubblico e il tuo profilo visitatore.
...
La chiamata *`SiteCatalyst: purchase`* non può essere utilizzata per i dati di traffico dell&#39;algoritmo di acquisto. Utilizza invece la chiamata *`orderConfirmPage`* .

## Verifica la priorità dell&#39;attività {#section_3D0DD07240F0465BAF655D0804100AED}

Le attività basate su moduli create con [!DNL Target Standard/Premium] potrebbero entrare in conflitto con le attività create nell&#39;interfaccia utente [!DNL Target Classic] che hanno la stessa priorità e utilizzano la stessa richiesta [!DNL Target].
...
## Il contenuto JavaScript distribuito dalla richiesta globale [!DNL Target] non viene caricato quando si utilizza mbox.js. {#section_03EC9B9C410B4F52A7FCD81840311709}

Effettua l’aggiornamento a [!DNL mbox.js] versione 58 o successiva.

La versione 58 o successiva di mbox.js esegue il contenuto non-JavaScript per la richiesta globale [!DNL Target] subito dopo il tag HTML `BODY` . Il contenuto JavaScript all’interno dei tag `<script>` per la richiesta globale [!DNL Target] viene eseguito dopo l’attivazione dell’evento `DOMContentLoaded` . Questo ordine di consegna del contenuto garantisce che il contenuto JavaScript per la richiesta globale [!DNL Target] venga consegnato e riprodotto correttamente.

## Il cookie di Target non viene impostato {#section_77AFEB541C0B495EB67E29A4475DF960}
...
Puoi controllare la risposta [!DNL Target] nel browser o utilizzare mboxTrace. Per ulteriori informazioni sugli URL validi, consulta [https://tools.ietf.org/html/std66](https://tools.ietf.org/html/std66).

## Le richieste Target non vengono attivate sul mio sito.
...
### Mbox Trace ![Badge tutorial](/help/assets/tutorial.png)
