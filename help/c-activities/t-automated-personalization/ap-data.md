...
description: Scopri quali Adobi di dati [!DNL Target] raccolgono e utilizzano per creare gli algoritmi di personalizzazione nelle attività di Automated Personalization (AP) e Targeting automatico (AT).
title: Quali dati vengono raccolti per creare gli algoritmi di personalizzazione?
...
# ![PREMIUM](/help/assets/premium.png)[!DNL Target] Raccolta di dati per gli algoritmi di personalizzazione 

[!DNL Adobe Target] raccoglie e utilizza automaticamente una varietà di dati per creare gli algoritmi di personalizzazione nelle attività di  [!UICONTROL Automated Personalization]  (AP) e  [!UICONTROL Targeting automatico]  (AT). Quando un visitatore accede a un’attività di Personalizzazione automatizzata o Targeting automatico, viene trasmessa un’istantanea di informazioni a un set di &quot;record di formazione&quot; (i dati del visitatore su cui gli algoritmi di personalizzazione impareranno).
...
| Parametri URL di riferimento | In generale, l’URL di riferimento è l’URL che fa riferimento a una pagina particolare che ha avviato la chiamata Target.<br>Questa variabile può essere influenzata dall’attività degli utenti sul sito e dall’implementazione tecnica del sito. | `Custom - [Referring URL Parameter] - [Parameter value]` | Dati personalizzati |
...
| Visitor Profile - Last Target Interaction | Specifica l’ora dell’ultima interazione con Target. L’interazione si verifica su ogni richiesta [!DNL Target] perché l’implementazione corrente di [!DNL Target] aggiorna il profilo su ogni richiesta. | Doppio, millisecondi |
...
| Visitor Profile - Start of Current Visit | Specifica l’ora in cui è iniziata la visita/sessione corrente con Target. La visita con Target può essere avviata senza accedere a un’attività. È necessaria solo una chiamata a qualsiasi richiesta [!DNL Target]. Potrebbe trascorrere un po’ di tempo prima che il visitatore acceda all’attività e venga acquisita l’istantanea. | Doppio, millisecondi |
