...
Informazioni sull’utilizzo dei dati dei clienti aziendali provenienti dai database di gestione delle relazioni con i clienti (Customer Relationship Management, CRM) per il targeting del contenuto in [!DNL Adobe Target] utilizzando gli attributi del cliente in [!DNL Adobe Enterprise Cloud People] servizio.

I dati dei clienti aziendali raccolti attraverso più origini e memorizzati all&#39;interno dei database CRM possono essere utilizzati in [!DNL Target] per fornire in modo strategico i contenuti più rilevanti ai clienti, specificatamente concentrandosi sui clienti fidelizzati. Tipi di pubblico e attributi del cliente in [!DNL People] Questo servizio (già Profili e pubblico) unisce le funzioni di raccolta e analisi dei dati a quelle di test e ottimizzazione, rendendo immediatamente fruibili i dati e le informazioni.

## Panoramica degli attributi del cliente {#section_B4099971FA4B48598294C56EAE86B45A}

[Attributi del cliente](https://experienceleague.adobe.com/docs/core-services/interface/services/customer-attributes/attributes.html) nel [!DNL People] il servizio fa parte del [!DNL Adobe Experience Cloud] e fornisce alle aziende uno strumento per inviare i dati dei propri clienti al [!DNL Experience Cloud] piattaforma.
...
![esempio di crs](/help/main/c-target/c-visitor-profile/assets/crs.png)

Considera le seguenti informazioni mentre lavori con gli attributi del cliente e [!DNL Target]:

* Ci sono alcuni requisiti prerequisiti che è necessario soddisfare prima di poter utilizzare [!UICONTROL Attributi del cliente] funzionalità in [!DNL People] servizio. Per ulteriori informazioni, consulta &quot;Prerequisiti per il caricamento di attributi del cliente&quot; in [Attributi del cliente](https://experienceleague.adobe.com/docs/core-services/interface/customer-attributes/attributes.html#section_BD38693AFBF34926BA28E964963B4EA0) nel *Documentazione di Experience Cloud Services e Administration*.
* Presta attenzione alle limitazioni relative al caricamento di file, come documentato in [Informazioni su file di dati e origini dati per Attributi del cliente](https://experienceleague.adobe.com/docs/core-services/interface/services/customer-attributes/crs-data-file.html?lang=it) nel *Guida ai componenti dell’interfaccia centrale di Experience Cloud*. Best practice:

   * Carica singoli file di grandi dimensioni (all&#39;interno del [limiti specificati](https://experienceleague.adobe.com/docs/core-services/interface/services/customer-attributes/crs-data-file.html?lang=it)). I singoli file di grandi dimensioni sono preferiti rispetto a più file di piccole dimensioni.
...
* [!DNL Adobe] non garantisce che il 100% dei dati dell&#39;attributo del cliente (profilo visitatore) provenienti dai database di gestione delle relazioni con i clienti sia integrato nel [!DNL Experience Cloud] e, quindi, essere disponibile per l’utilizzo per il targeting in [!DNL Target]. Nella progettazione corrente, esiste la possibilità che una piccola percentuale di dati (fino allo 0,1% dei batch di produzione di grandi dimensioni) potrebbe non essere integrata.
* Durata dei dati degli attributi del cliente importati da [!DNL Experience Cloud] a [!DNL Target] dipende dalla durata del profilo del visitatore, che è di 14 giorni per impostazione predefinita. Per ulteriori informazioni, consulta [Durata profilo visitatore](/help/main/c-target/c-visitor-profile/visitor-profile-lifetime.md#concept_D9F21B416F1F49159F03036BA2DD54FD).
* Se il `vst.*` I parametri sono l’unica cosa che identifica il visitatore; il profilo &quot;autenticato&quot; esistente non verrà recuperato fintanto che `authState` è NON AUTENTICATO (0). Il profilo entra in gioco solo se `authState` viene modificato in AUTENTICATO (1).

   Ad esempio, se `vst.myDataSource.id` Il parametro viene utilizzato per identificare il visitatore (dove `myDataSource` è l’alias dell’origine dati) e non è presente alcun ID MCID o di terze parti, utilizzando il parametro `vst.myDataSource.authState=0` non recupera il profilo che potrebbe essere stato creato tramite un’importazione di Attributi del cliente. Se il comportamento desiderato è quello di recuperare il profilo autenticato, il `vst.myDataSource.authState` deve avere il valore 1 (AUTENTICATO).
...
1. In [!DNL Adobe Experience Cloud], fare clic sull&#39;icona del menu ( ![icona del menu](/help/main/c-target/c-visitor-profile/assets/menu-icon.png) ) quindi fai clic su **[!UICONTROL Persone]**.

   ![Persone](/help/main/c-target/c-visitor-profile/assets/people.png)

1. Fai clic su **[!UICONTROL Attributi del cliente]** scheda.
...
Le istruzioni dettagliate per il completamento di ciascuna delle seguenti attività sono reperibili in [Creare un&#39;origine attributo del cliente e caricare il file di dati](https://experienceleague.adobe.com/docs/core-services/interface/services/customer-attributes/t-crs-usecase.html) nel *Documentazione di Experience Cloud Services e Administration*.
...
1. Creazione di una sorgente attributo e caricamento del file di dati.
   ...
   * **HTTPS:** È possibile trascinare il file di dati .csv o fare clic su **[!UICONTROL Sfoglia]** per caricare dal file system.
   * **FTP:** Fai clic sul collegamento FTP per [caricare il file tramite FTP](https://experienceleague.adobe.com/docs/core-services/interface/customer-attributes/t-upload-attributes-ftp.html). Il primo passo è quello di fornire una password per il server FTP fornito da Adobe. Specifica la password, quindi fai clic su **[!UICONTROL Fine]**.
   ...
   Ora puoi trasferire il file CSV/ZIP/GZIP al server FTP. Dopo il corretto trasferimento dei file, creare un file con lo stesso nome e `.fin` estensione. Trasferisci questo file vuoto al server. Questo indica una fine del trasferimento e [!DNL Experience Cloud] inizia a elaborare il file di dati.
...
1. Convalida dello schema.
   ...
   Clic **[!UICONTROL Salva]** al termine della convalida dello schema. Il tempo di upload dei file varia a seconda delle dimensioni.
...
## Utilizzare gli attributi del cliente in [!DNL Target] {#section_107E3A0F0EC7478E82E6DBD17B30B756}
...
Superato `mbox3rdPartyId` come parametro per la mbox globale all’interno del `targetPageParams()` metodo. Il valore di `mbox3rdPartyId` deve essere impostato sull&#39;ID cliente presente nel file di dati CSV.
...
### Problema 2: attributi non elencati nella Libreria tipi di pubblico in [!DNL Target], anche dopo diversi giorni
...
* L&#39;ID del visitatore non è stato passato correttamente. L&#39;ID è stato passato `mboxMCGVID` invece di `setCustomerId`.
...
Se non riesci a trovare i dati Attributi del cliente in Target, accertati che l’importazione sia avvenuta entro l’ultimo *x* giorni in cui *x* è il target [Durata profilo visitatore](/help/main/c-target/c-visitor-profile/visitor-profile-lifetime.md) (14 giorni per impostazione predefinita).
...
Questo video illustra come importare dati CRM offline, da help desk o punti vendita e altri dati di marketing in [!DNL Experience Cloud People] e associarlo ai visitatori utilizzando i loro ID noti.
