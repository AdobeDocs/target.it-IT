...
* **Comprimi la navigazione a sinistra.** La navigazione a sinistra è ora comprimibile ed espandibile per tutte le schede. Se compresse, le icone vengono visualizzate con le descrizioni. Quando espansi, vengono visualizzati sia le icone che il testo. Questo miglioramento fornisce agli utenti maggiore spazio sullo schermo quando necessario, mantenendo al contempo un accesso rapido alle opzioni di navigazione. (TGT-54608)
...
* Il menu a discesa **[!UICONTROL Workspaces]è limitato a 20 nella sezione [!UICONTROL Users].** Questa correzione risolve un problema a causa del quale il menu a discesa [!UICONTROL Workspaces] nella sezione [!UICONTROL Administration] > [!UICONTROL Users] mostrava solo 20 aree di lavoro, anche quando un utente aveva accesso a molte altre aree. Anche la chiamata GraphQL sottostante per `licenseGroups` è stata limitata a 20 risultati, causando la visualizzazione di un elenco incompleto nell&#39;interfaccia utente nonostante l&#39;utente abbia accesso a più aree di lavoro nell&#39;organizzazione. L’aggiornamento rimuove questo limite rigido in modo che venga restituito e visualizzato correttamente l’intero set di spazi di lavoro disponibili. (TGT-53820)
...
* **È stato risolto un problema a causa del quale la ricerca non veniva eseguita nelle sottocartelle.** È stato risolto un problema a causa del quale la ricerca delle offerte non restituiva risultati dalle sottocartelle nell&#39;interfaccia utente aggiornata. I clienti potevano trovare un’offerta solo se si spostavano manualmente nella cartella in cui era memorizzata, rendendo il comportamento di ricerca incoerente con le funzionalità API. La funzione di ricerca ora supporta la ricerca ricorsiva nelle cartelle, in modo che i clienti possano individuare le offerte senza dover aprire ogni cartella singolarmente. (TGT-51954)
...
* **Il salvataggio delle modifiche nelle attività ora funziona come previsto.** È stato risolto un problema relativo alle autorizzazioni nella nuova interfaccia utente di creazione che impediva agli utenti di salvare le modifiche durante la modifica degli obiettivi e delle impostazioni avanzate nelle attività. Gli utenti interessati visualizzavano una barra multifunzione di errore rossa e un messaggio &quot;Forbidden.Resource&quot;, nonostante disponessero dell’accesso appropriato. (TGT-53816)
* **L&#39;interfaccia utente di VEC ora mantiene le modifiche dell&#39;esperienza nelle viste.** sono stati risolti più problemi nella VEC aggiornata che influivano sullo sviluppo dell&#39;esperienza. Le modifiche non persistevano correttamente, in particolare quando si utilizzavano le offerte HTML o si passava da una visualizzazione all’altra. (TGT-53825)
...
* **I clic del pulsante in modalità [!UICONTROL Composer] non attivano più il reindirizzamento della pagina.**: è stato risolto un problema nell&#39;interfaccia utente VEC aggiornata a causa del quale facendo clic su un pulsante nella modalità [!UICONTROL Composer] si verificava un reindirizzamento imprevisto all&#39;URL di destinazione del pulsante. Ciò ha impedito agli utenti di modificare gli elementi di call-to-action (CTA) e ha interrotto il flusso di lavoro di creazione. (TGT-53137)
...
* La modalità **[!UICONTROL Design]in VEC ora blocca la navigazione dei collegamenti per i componenti modificabili.** ha risolto un problema nel file VEC aggiornato in cui gli elementi selezionabili, ad esempio pulsanti e collegamenti, attivavano il reindirizzamento delle pagine anche in modalità [!UICONTROL Design]. Questo comportamento ha simulato la modalità [!UICONTROL Browse] e ha impedito agli utenti di modificare i componenti chiave. (TGT-53696)
...
In precedenza, il termine &quot;Dettagli&quot; era tradotto erroneamente come &quot;详情&quot; nelle impostazioni internazionali per il cinese semplificato (zh_CN), in violazione delle linee guida stabilite sulla terminologia. Questo problema è stato corretto in &quot;详细信息&quot; per garantire la coerenza con il database terminologico. (TGT-53741)

+++

+++[!UICONTROL Recommendations]

* **Impossibile individuare e selezionare le caselle dei suggerimenti nel VEC.** Dopo aver aggiunto un&#39;offerta di suggerimenti in (VEC), facendo clic sulla modifica nel pannello a sinistra non è stato possibile evidenziare o scorrere fino alla casella dei suggerimenti corrispondente nella pagina. Questo ha reso difficile individuare e modificare l&#39;offerta, soprattutto se nascosta sotto i selettori o con uno stile minimo. Facendo clic su una modifica del consiglio ora viene evidenziato e scorrere correttamente l’elemento associato, migliorando l’usabilità e l’efficienza della modifica nel processo aggiornato di creazione delle attività. (TGT-52571)
* **I selettori dei suggerimenti sono stati riscritti in modo errato dopo il salvataggio di un&#39;attività.** Quando si aggiungeva un suggerimento a un elemento nella VEC, il selettore era inizialmente corretto, ma dopo aver salvato e riaperto l&#39;attività, è passato a un selettore generico. I tentativi di ripristino manuale del selettore originale hanno generato errori di convalida. I selettori di suggerimenti ora persistono in modo accurato dopo il salvataggio, garantendo una targeting e una modificabilità affidabili nel processo di creazione delle attività aggiornato. (TGT-53709)
* **Impossibile modificare il contenuto dei criteri durante la modifica di un&#39;attività esistente.** Durante la modifica di un&#39;attività, la sezione del contenuto [!UICONTROL Criteria] è apparsa disabilitata, con i pulsanti disattivati e che non rispondono. Questo problema è stato risolto verificando che [!UICONTROL Criteria] configurazioni siano completamente modificabili durante gli aggiornamenti delle attività. I clienti possono ora modificare il contenuto di [!UICONTROL Criteria] senza dover cambiare le selezioni o utilizzare soluzioni alternative, migliorando la flessibilità e l&#39;usabilità nel processo aggiornato di creazione delle attività. (TGT-53812)
...
* **La generazione di offerte ad hoc nelle attività A[!UICONTROL utomated Personalization] ha causato incongruenze nei rapporti.** L&#39;utilizzo della funzionalità Genera offerte ad-hoc nelle attività di [!UICONTROL Automated Personalization] (AP) ha causato report non accurati. In particolare, gli ID offerta venivano riutilizzati tra le posizioni, causando l’erronea attribuzione o la sovrascrittura dei dati di reporting. Le offerte ad-hoc vengono ora generate con identificatori distinti per località, garantendo tracciamento e reporting accurati su tutte le esperienze configurate. (TGT-53757)
...
* **I report si sono arrestati in modo anomalo dopo aver interagito con la barra di scorrimento.** Se si fa clic sulla barra di scorrimento nella scheda [!UICONTROL Reports], la pagina si arresta in modo anomalo, insieme a un errore JavaScript:
...
* **Nei report non è stata visualizzata la metrica principale.** La metrica principale, configurata come metrica di conversione utilizzando un mbox, non era presente nei report di attività. La ricerca in base al nome della metrica o della casella di posta non ha prodotto risultati, impedendo la visibilità nei dati delle prestazioni chiave. Le metriche primarie ora vengono visualizzate correttamente nella scheda [!UICONTROL Reports], garantendo un tracciamento e un&#39;analisi accurati delle prestazioni della campagna. (TGT-53773)
...
* **Se si fa clic su breadcrumbs in VEC, il menu Modifica non viene visualizzato in modo uniforme.**
Quando si selezionano gli elementi HTML tramite gli indicatori di percorso (VEC), il menu Modifica a intermittenza non viene visualizzato o scompare rapidamente, rendendo inaffidabile la selezione degli elementi. Il menu Modifica ora viene visualizzato in modo uniforme durante la navigazione tramite percorsi di navigazione, migliorando il flusso di lavoro di selezione degli elementi nel processo aggiornato di creazione delle attività. (TGT-52873)
* **Il menu di scelta rapida non è stato visualizzato in modo intermittente nella VEC.** Il menu di scelta rapida nell&#39;interfaccia utente VEC aggiornata non veniva visualizzato in modo uniforme quando si faceva clic sugli elementi, rendendo difficile l&#39;accesso alle opzioni di modifica. Il menu di scelta rapida visualizza ora in modo affidabile la selezione degli elementi, migliorando il flusso di lavoro di modifica e l’usabilità generale nel processo aggiornato di creazione delle attività. (TGT-53015)
* **Impossibile visualizzare il menu di scelta rapida per alcuni elementi nel Compositore esperienza visivo.** Il menu di scelta rapida non è stato visualizzato quando si selezionano elementi specifici nel file VEC aggiornato, rendendo difficile l&#39;applicazione delle modifiche. Il menu di scelta rapida viene ora visualizzato in modo uniforme per tutti gli elementi supportati, migliorando l’affidabilità e la fruibilità dell’esperienza di modifica nel flusso di lavoro aggiornato di creazione delle attività. (TGT-53248)
* **Il menu di scelta rapida è scomparso al primo clic durante l&#39;utilizzo di breadcrumbs in VEC.** La selezione di un elemento padre tramite i percorsi nel VEC ha causato la visualizzazione rapida del menu di scelta rapida, che è quindi scomparso, rendendo difficile l&#39;accesso alle opzioni di modifica. Il menu di scelta rapida ora rimane visibile e funzionale quando si naviga tra gli elementi, migliorando l’affidabilità del flusso di lavoro di selezione degli elementi nel processo aggiornato di creazione delle attività. (TGT-53424)
* **Il menu di scelta rapida non è stato visualizzato per gli elementi di primo livello nella VEC.** La selezione di elementi di primo livello, ad esempio i tag `<div>` o `<main>`, tramite il percorso nel VEC non ha attivato il menu di scelta rapida, impedendo ulteriori azioni di modifica. Il menu di scelta rapida viene ora visualizzato in modo uniforme per tutti gli elementi supportati, inclusi i contenitori di primo livello, migliorando la flessibilità e l’usabilità del flusso di lavoro di creazione delle attività. (TGT-53770)
* **Gli elementi di una pagina specifica non erano modificabili nella VEC.** Impossibile selezionare o modificare alcuni elementi nella pagina nel Compositore esperienza visivo aggiornato. Questo problema era isolato in quella pagina e non interessava altre pagine all’interno dello stesso account. Tutti gli elementi della pagina ora sono selezionabili e modificabili come previsto, ripristinando la funzionalità completa nel flusso di lavoro per la creazione di attività. (TGT-53353)
...
Questa versione contiene i seguenti aggiornamenti e correzioni:
...
* **Acquisizione proattiva dei dati dei clienti [!DNL Target] selezionati durante la creazione delle attività**: è stato migliorato il processo di creazione delle attività abilitando la raccolta proattiva di report, contenuti e schermate dei clienti [!DNL Target]. Questo miglioramento risolve le lacune nei dati identificate nei casi d’uso esistenti e contribuisce a garantire informazioni più accurate durante la configurazione delle attività e degli esperimenti. (TGT-52415)
* **Le attività del punto di accesso non hanno recuperato i dati pronti per il modello nella sezione [!UICONTROL Reports]**: i clienti che visualizzano le attività del punto di accesso di Automated Personalization nella sezione [!UICONTROL Reports] non sono stati in grado di visualizzare gli indicatori pronti per il modello a livello di gruppo di report e di offerta. Questo problema si è verificato perché i dati pronti per il modello non venivano recuperati correttamente dal back-end. La funzionalità è stata ripristinata e i dati pronti per il modello ora vengono visualizzati come previsto. (TGT-53600 e TGT-53601)
...
* **È stato risolto un problema che causava un filtro sensibile a maiuscole e minuscole nella ricerca avanzata di [!UICONTROL Product Catalog Search]**: il filtro della ricerca avanzata nella pagina [!UICONTROL Product Catalog Search] ignora ora correttamente la distinzione tra maiuscole e minuscole, in linea con il comportamento dei servizi di back-end e GraphQL. Questo aggiornamento garantisce risultati di suggerimenti coerenti e precisi per i clienti, indipendentemente dalle maiuscole e minuscole. (TGT-53585)
...
+++Vedi i dettagli
* **Impossibile caricare i report per il gruppo di destinatari desktop a causa di un errore di nome di gruppo di destinatari non valido**: i clienti hanno riscontrato un errore di GraphQL durante il tentativo di visualizzare i report per un gruppo di destinatari nel processo di creazione delle attività. Il sistema ha restituito un messaggio di tipo &quot;Nome pubblico non valido: XXXXX&quot; che impediva l’accesso ai dati di reporting. I rapporti ora vengono caricati correttamente per il pubblico del desktop. (TGT-53371)
...
* **[!UICONTROL Graph view]nella sezione [!UICONTROL Reports] non ha visualizzato i valori di[!DNL Analytics]**: i clienti che accedono a [!UICONTROL Graph view] nella sezione Reporte hanno rilevato dati mancanti, con tutti i valori visualizzati come zero. Questo problema è stato causato da un recupero di dati non corretto da [!UICONTROL Analytics]. [!UICONTROL Graph view] ora visualizza valori accurati come previsto. (TGT-52792)
+++
...
* **La cancellazione di tutti i valori di data tramite Backspace ha causato l&#39;arresto anomalo della pagina**: le attività di pianificazione dei clienti nella sezione [!UICONTROL Goals & Settings] hanno subito un arresto anomalo durante l&#39;utilizzo di Backspace per cancellare tutti i valori dai campi &quot;[!UICONTROL Specified Date & Time]&quot;. Questo problema è stato causato da un errore di riferimento null nella logica di gestione delle date. La pagina ora gestisce gli input di date vuote in modo ordinato, senza arresti anomali. (TGT-53624)
* **Nessun prodotto visualizzato in [!UICONTROL Product Catalog Search] a causa di un payload non valido**: i clienti che accedono alla sezione [!UICONTROL Recommendations] in [!UICONTROL Product Catalog Search] hanno rilevato risultati vuoti causati da un payload di GraphQL non valido. Questo errore di back-end ha impedito il caricamento corretto dei dati del prodotto. I prodotti ora vengono visualizzati come previsto nell&#39;interfaccia utente aggiornata. (TGT-53630)
...
* **È stato risolto un problema che impediva ai clienti di salvare un&#39;attività a causa di un errore InvalidProperty.Json**: i clienti hanno riscontrato un errore durante il tentativo di salvare un&#39;attività nell&#39;interfaccia utente aggiornata, anche senza apportare modifiche. Il messaggio di errore indicava una struttura JSON non valida: &quot;Json non valido. Nome di proprietà &#39;content&#39; non riconosciuto. Posizione: riga - 1, colonna - 432.&quot; Questo problema era causato da una proprietà non riconosciuta nel payload della richiesta ed è stato ora risolto. I clienti possono salvare le attività senza riscontrare questo errore. (TGT-53513)
...
* **Comportamento corretto dei campi degli elementi rilevati in modalità Fai clic su Traccia per impedire input non salvati e migliorare la chiarezza**: è stato risolto un problema nel processo di creazione delle attività in cui il campo [!UICONTROL Tracked Element] in modalità [!DNL Click Track] era modificabile, ma non manteneva il nome immesso, causando confusione per i clienti. Il campo è ora disabilitato per impedire l’input non salvato e la denominazione degli elementi selezionati è stata chiarita per migliorare la configurazione dell’obiettivo e la precisione del tracciamento.** (TGT-53458)
...
+++Visualizza dettagli
* **Problema risolto nel processo di creazione delle attività in cui gli spazi iniziali negli URL causavano collegamenti QA interrotti**: è stato risolto un problema nel processo di creazione delle attività in cui gli spazi iniziali nell&#39;URL delle attività non venivano tagliati correttamente durante il salvataggio. Questo causava collegamenti QA non validi e formattazione errata nel back-end. Dopo l’aggiornamento, gli URL vengono ora salvati correttamente, evitando collegamenti interrotti e migliorando l’affidabilità dei flussi di lavoro di controllo qualità per i clienti. (TGT-53427)
...
* **È stato risolto un problema nel processo di creazione attività a causa del quale i messaggi di prodotto non venivano visualizzati nella visualizzazione consigli**: è stato risolto un problema nel processo di creazione attività a causa del quale la colonna [!UICONTROL Message] nella visualizzazione [!DNL Recommendations] non visualizzava i dati di prodotto, anche se i messaggi erano presenti. I clienti dovevano rimuovere manualmente e aggiungere nuovamente la colonna per rivelare temporaneamente il contenuto, che spesso scompariva di nuovo durante lo scorrimento o la ricerca. Questo aggiornamento ripristina la visibilità coerente dei messaggi del prodotto, migliorando la navigazione del catalogo e i flussi di lavoro di revisione. (TGT-52777)
* **Problema risolto nel processo di creazione delle attività in cui la selezione di &#39;Tutti i gruppi host&#39; non ha restituito risultati nella visualizzazione dei suggerimenti**: è stato risolto un problema nel processo di creazione delle attività in cui la selezione dell&#39;ambiente &quot;Tutti i gruppi host&quot; nella visualizzazione [!DNL Recommendations] non ha restituito risultati. I clienti possono ora visualizzare i dati dei prodotti in tutti i gruppi host come previsto, migliorando la visibilità e l’accuratezza del filtro durante la configurazione dell’attività. (TGT-53006)
...
+++Visualizza dettagli
* Sono stati risolti diversi problemi con l’interfaccia utente aggiornata, inclusi errori durante l’eliminazione di tipi di pagina a causa della spaziatura nei valori di input, della copia inaffidabile delle attività tra aree di lavoro e del malfunzionamento delle regole di recapito delle pagine negli ambienti QA. (TGT-52703)
* È stato risolto un problema nell&#39;interfaccia utente [!DNL Target] aggiornata a causa del quale gli utenti con il ruolo [!UICONTROL Editor] potevano accedere e tentare di modificare le attività dinamiche, che devono essere limitate. Le schermate Elenco attività e Panoramica mostravano in modo errato le opzioni di modifica per le attività dal vivo, causando potenziali modifiche non intenzionali. (TGT-53055)
* È stato risolto un problema nell&#39;interfaccia utente di [!UICONTROL Advanced Search] a causa del quale la selezione del valore &quot;è presente&quot; o &quot;non è presente valore&quot; comportava che gli operatori richiedessero l&#39;immissione di un operando, che non dovrebbe essere richiesto per queste condizioni. (TGT-51961)
* È stato risolto un problema nell’interfaccia utente aggiornata a causa del quale i tentativi di copiare le attività dall’area di lavoro di staging all’area di lavoro di produzione continuavano a non riuscire, anche negli ambienti sandbox. I clienti hanno riscontrato diversi messaggi di errore, tra cui &quot;Destinatari anonimi già utilizzati dall&#39;attività&quot; e &quot;ID pubblico non valido&quot;, nonostante l&#39;utilizzo di configurazioni valide e l&#39;utilizzo delle autorizzazioni appropriate per l&#39;area di lavoro. (TGT-52394)
...
* È stato risolto un problema a causa del quale la copia di un&#39;attività che includeva offerte generava offerte duplicate create nella nuova attività. Questo comportamento ha causato confusione e confusione inutili, in particolare quando si spostano le attività tra le aree di lavoro. La correzione garantisce che le offerte [!DNL Target] vengano copiate correttamente nell&#39;area di lavoro di destinazione senza duplicazioni, semplificando la gestione delle attività e migliorando l&#39;efficienza del flusso di lavoro. (TGT-53454)
...
+++Vedi i dettagli
* È stato risolto un problema a causa del quale le modifiche apportate alle esperienze in [!UICONTROL Visual Experience Composer] non erano visibili o risultavano incoerenti nell&#39;interfaccia utente. (TGT-TGT-53381)
...
* È stato corretto un errore a causa del quale i clienti riscontravano errori di convalida durante la configurazione di [!UICONTROL page delivery] regole nelle attività VEC. In particolare, gli operatori come &quot;uguale a uno di&quot; e &quot;non uguale a nessuno di&quot; attivano &quot;Input non valido per il tipo di operatore&quot; quando si immettono valori di testo, anche se il testo deve essere supportato. (TGT-52629)
...
+++Vedi i dettagli
* È stato risolto un problema a causa del quale la chiamata di `triggerView` tramite codice personalizzato causava un ciclo infinito. (TGT-52885)
* È stato risolto un problema che causava mancate corrispondenze tra `LocalIds` definito per le attività e `LocalIds` utilizzato nelle definizioni dell&#39;esperienza. (TGT-52669)
* È stato risolto un problema a causa del quale nella pagina dell&#39;attività [!UICONTROL Overview] mancavano i nomi delle metriche. Veniva visualizzato solo &quot;Offer&quot; anziché il nome della metrica corretto. (TGT-53054)
...
* È stato risolto un problema a causa del quale [suggerimenti come offerte](/help/main/c-recommendations/recommendations-as-an-offer.md) nelle attività [!UICONTROL A/B Test] non venivano aggiornati correttamente quando si modificavano raccolte o promozioni. (TGT-52884)
...
* È stata migliorata la gestione delle offerte ad hoc quando si passa da un’area di lavoro a un’altra.
   ...
   * Non si verificano modifiche quando si rimane nella stessa area di lavoro. (TGT-53079)
...
* È stato risolto un problema che si verificava durante la modifica delle proprietà tra le aree di lavoro.
   ...
   * Se rimani nella stessa area di lavoro o passi da un&#39;area di lavoro non predefinita a quella predefinita o a un&#39;altra, tutto rimane inalterato. (TGT-53078)
* La logica di convalida dell&#39;entità è stata aggiornata in modo da rispettare il contesto dell&#39;area di lavoro originale dell&#39;attività. Le entità come [!UICONTROL Experience Fragments] (XF) sono ora convalidate in base all&#39;area di lavoro in cui l&#39;attività è stata originariamente creata. Ad esempio, se un file XF esiste nel workspace predefinito e l&#39;attività viene copiata dal workspace X al workspace Y, la convalida viene comunque eseguita finché il file XF è valido nel workspace originale (predefinito). (TGT-53196)
...
+++Vedi i dettagli
* È stato risolto un problema a causa del quale la copia di un&#39;attività esistente e la modifica dell&#39;origine dei report in [!DNL Adobe Analytics] (A4T) generavano un errore di tipo &quot;Input utente non valido&quot;. L&#39;errore è stato attivato quando alcune azioni della metrica incompatibili con il reporting [!DNL Analytics], ad esempio `restart_same_experience`, `restart_random_experience` e `restart_new_experience`, sono state mantenute dall&#39;attività originale. (TGT-52900)
* È stato risolto un problema che impediva ai clienti di creare o salvare un&#39;attività quando si selezionava [!DNL Adobe Analytics] (A4T) come origine report nel passaggio [!UICONTROL Goals & Settings]. Il problema si è verificato in modo specifico quando si seleziona una metrica [!UICONTROL Custom Event] (ad esempio, &quot;Evento personalizzato 16&quot;) e si verifica il seguente errore: &quot;Input utente non valido&quot;. (TGT-52910)
* È stato risolto un problema a causa del quale, facendo clic sul collegamento &quot;[!UICONTROL View in Analytics]&quot;, gli utenti venivano reindirizzati alla home page anziché alla dashboard [!DNL Analytics] prevista. (TGT-53092 e TGT-53093)
...
* È stato risolto un problema durante la visualizzazione di un&#39;attività [!DNL Recommendations] nell&#39;interfaccia utente [!UICONTROL Overview] aggiornata. La sezione [!UICONTROL Goals & Settings] non viene caricata quando [!DNL Adobe Analytics] (A4T) è selezionato come origine dei report. Veniva visualizzato il seguente messaggio di errore: &quot;Si è verificato un errore. Impossibile completare la richiesta. Se il problema persiste, contatta l’Assistenza clienti Adobe.” (TGT-52999)
...
* Sono stati risolti diversi problemi nel flusso di lavoro di creazione attività relativi alla denominazione delle offerte e alla traduzione dei contenuti in [!UICONTROL Automated Personalization] (AP) e [!UICONTROL Multivariate Testing] (MVT) attività:
   ...
   * La creazione di più offerte HTML con lo stesso nome (ad esempio, &quot;Esperienza&quot;) ha generato un errore di tipo &quot;I nomi di offerte duplicati non sono consentiti&quot;, ma l’interfaccia utente non indicava chiaramente quali offerte stavano causando il conflitto.
...
* È stato risolto un problema che impediva la creazione di una progettazione [!DNL Recommendations]. Facendo clic su [!UICONTROL Create] è stato attivato il messaggio: &quot;Dovrebbe essere utilizzata almeno 1 variabile di entità all&#39;interno dello script.&quot; (TGT-52395 e TGT-52899)
...
* È stato risolto un problema in [!UICONTROL Visual Experience Composer] (VEC) aggiornato che causava un errore di tipo &quot;Input non valido&quot; quando gli utenti tentavano di aggiungere frammenti di esperienza. (TGT-52701)
...
* È stato risolto un problema a causa del quale l’interfaccia utente non consentiva l’assegnazione manuale di un ID ambiente a un criterio. Viene invece impostato come valore predefinito l&#39;ID del gruppo host [!UICONTROL Product Catalog Search]. Questa correzione garantisce che le modifiche ai criteri vengano ora applicate in tutti gli ambienti, non solo per impostazione predefinita. (TGT-52817)
...
* È stato risolto un problema che impediva il salvataggio di [!DNL Recommendations] progetti utilizzando gli array di velocità. Il messaggio di errore &quot;Deve essere utilizzata almeno 1 variabile di entità nello script&quot; è stato attivato in modo errato. (TGT-52734)
...
* È stato risolto un problema che impediva ai clienti di eliminare o rimuovere offerte nelle attività di [!UICONTROL Automated Personalization] (PA). (TGT-52690)
* È stato risolto un problema che causava un comportamento incoerente di qualificazione delle attività nelle attività a più pagine. (TGT-52694)
* È stato risolto un problema a causa del quale la pagina [!UICONTROL Overview] dell&#39;attività visualizzava un URL non valido per [!UICONTROL Activity Location]. (TGT-52695)
...
* È stato risolto un problema che impediva l’aggiunta o la modifica di URL in modalità QA. (TGT-51941)
* È stata aggiunta un&#39;impostazione di traffico in modalità QA in [!UICONTROL Reports] > [!UICONTROL Report Settings] ( ![icona Impostazioni report](/help/main/assets/icons/Setting.svg) ) per allinearsi alle funzionalità della precedente interfaccia utente [!DNL Target]. (TGT-52228 e TGT-52329)
...
* È stato risolto un problema a causa del quale [!DNL Target] non avvisava i clienti quando modificava un&#39;attività e un elemento del sito Web modificato. (TGT-52100)
...
* È stato risolto un problema che causava discrepanze negli elenchi di gruppi di destinatari tra la pagina delle impostazioni [!UICONTROL Activity] e la pagina della panoramica [!UICONTROL Reporting]. (TGT-52203)
* È stato risolto un problema che impediva di aggiungere una nuova pagina a un’attività a causa di un errore di input dell’utente non valido. (TGT-52263)
...
* È stato risolto un problema in [!UICONTROL Visual Experience Composer] (VEC) aggiornato in cui gli indicatori di percorso non venivano sempre visualizzati nella parte inferiore dell&#39;editor, causando difficoltà nella selezione precisa degli elementi. (TGT-52169)
...
* È stato risolto un problema a causa del quale l&#39;offerta predefinita veniva visualizzata in modo errato nell&#39;interfaccia utente di [!DNL Target] all&#39;apertura dell&#39;attività. (TGT-52198)
...
Questa versione include le seguenti correzioni e aggiornamenti:
...
* È stato corretto un errore che impediva ai clienti di aprire la finestra a comparsa delle informazioni sul pubblico per alcune attività [!UICONTROL Experience Targeting] (XT). (TGT-52049)
...
* È stata aggiunta la possibilità di consentire la duplicazione dei gruppi di destinatari all’interno di un’attività. (TGT-51200)
...
* L’interfaccia utente aggiornata non ha salvato pagine o gruppi di destinatari senza modifiche. Se i clienti hanno aggiunto nuove pagine o gruppi di destinatari a un&#39;attività senza apportare modifiche, [!DNL Target] ha eliminato i gruppi di destinatari non modificati al salvataggio. Le notifiche sono state aggiunte in posizioni pertinenti per informare gli utenti di questo comportamento. (TGT-52104)

### [!DNL Target Standard/Premium] 25.4.1 (giovedì 2 aprile 2025)

Questa versione include le seguenti correzioni e aggiornamenti:

* È stato risolto un problema che causava la scomparsa dei gruppi di destinatari delle esperienze dalle attività. (TGT-52003)
* È stato risolto un problema che causava elementi imprevisti durante il recapito. (TGT-52011)
* È stato risolto un problema che impediva ai clienti di visualizzare il gruppo di destinatari nel grafico di destinazione nella pagina di visualizzazione attiva[!UICONTROL r]e durante la modifica delle attività. (TGT-52050)
...
* È stato risolto un problema a causa del quale, quando si modifica un&#39;offerta di reindirizzamento, [!DNL Target] rimuove anche i selettori [!UICONTROL ClickTrack] associati a tale offerta. (TGT-51936)
...
* È stato risolto un problema che escludeva i contatori di ID locali dal payload durante le operazioni di creazione e modifica in batch. (TGT-51867)
...
* È stato risolto un problema a causa del quale [!UICONTROL Visual Experience Composer] (VEC) non riusciva ad applicare correttamente le modifiche a un&#39;esperienza nella prima modifica. (TGT-51843)
...
* Gestione degli errori aggiornata in VEC durante la creazione dell&#39;attività. (TGT-51759)
...
Utilizzando il sistema di progettazione [!DNL Adobe Spectrum] più recente, questo aggiornamento standardizza i modelli di progettazione incoerenti e introduce nuovi miglioramenti, tra cui i seguenti:

* **Gestione delle offerte in blocco**: seleziona ed elimina o sposta più offerte contemporaneamente.

* **[!UICONTROL Code Editor]aggiornamenti**: editor HTML e JSON aggiornati con evidenziazione della sintassi e numerazione delle righe.

* **Schede dell&#39;offerta migliorate**: informazioni rapide e schede dei dettagli migliorate per semplificare l&#39;accesso alle informazioni.

* **Ricerca e filtri permanenti**: aggiunge le opzioni di ricerca e filtro permanenti per la sessione.
...
* **Singola chiamata per la personalizzazione e la misurazione.**
...
Per ulteriori informazioni sulle versioni at.js, vedere [at.js versione details](https://experienceleague.adobe.com/en/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions){target=_blank} nella *Guida per sviluppatori Adobe Target*.

### Report di [!DNL Target] in [!DNL Adobe Customer Journey Analytics] (8 maggio 2024)

L&#39;integrazione tra [Adobe Customer Journey Analytics](https://experienceleague.adobe.com/en/docs/customer-journey-analytics){target=_blank} e [!DNL Target] offre potenti strumenti di analisi e risparmio di tempo per il programma di ottimizzazione.
...
* È stato risolto un problema che causava la visualizzazione non corretta delle attività [!UICONTROL Multivariate] (MVT) configurate con il report [!UICONTROL Analytics for Target] (A4T). (TGT-47490)
* È stato migliorato il messaggio di avviso visualizzato nel report quando come esperienza di controllo viene utilizzata un’esperienza senza traffico. (TGT-47537)
* Sono state aggiunte molte correzioni di backend e localizzazione.

### [!DNL Target] Standard/Premium 24.1.1 (22, 23 e 25 gennaio 2024)

Questa versione include i miglioramenti e le correzioni seguenti:

* Le attività [!UICONTROL Analytics for Target] (A4T) con metriche degli obiettivi dei ricavi non visualizzavano &quot;Ricavi&quot; come nome di colonna e la metrica dei ricavi non veniva visualizzata nel formato ($) nei report. Si trattava di un problema estetico che è stato risolto. (TGT-46995)
...
* È stato risolto un problema che impediva la ricerca di un&#39;attività nella pagina [!UICONTROL Activity] se il nome dell&#39;attività contiene una parentesi quadra ( [o] ). (TGT-44777)
...
* È stato risolto un problema a causa del quale le richieste POST di creazione di feed [!DNL Recommendations] non andavano a buon fine quando si utilizzava [!UICONTROL Analytics Classifications] con gruppi di report il cui nome conteneva punti. (TGT-44598)
...
>[!NOTE]
>
>Per garantire il recapito delle modifiche create in VEC, assicurati di utilizzare un SDK [!DNL Target] ([at.js](https://experienceleague.adobe.com/docs/target-dev/developer/client-side/at-js-implementation/target-atjs-versions.html?lang=it){target=_blank} o [Adobe Experience Platform Web SDK](https://experienceleague.adobe.com/docs/experience-platform/edge/release-notes.html?lang=it){target=_blank} (alloy.js)) con una versione successiva a 2.8.
...
| Funzione | Dettagli |
| --- | --- |
| [!DNL Adobe Experience Manager] Frammenti esperienza (AEM) | Gli aggiornamenti relativi alla funzionalità dei frammenti di esperienza AEM includono:<ul><li>È stata aggiunta la possibilità di filtrare i Frammenti di esperienza di AEM per tipo (HTML o JSON) nell&#39;elenco [!UICONTROL Offers]. (TGT-43121)</li><li>È stato risolto un problema che consentiva ai clienti di inserire offerte JSON [!UICONTROL Experience Fragment] quando utilizzavano VEC, che non è supportato. Le offerte JSON possono essere inserite solo quando si utilizza il modulo di composizione [!UICONTROL Form-Based Experience]. (TGT-43846)</li></ul>Per ulteriori informazioni, consulta i [Frammenti esperienza AEM](/help/main/c-experiences/c-manage-content/aem-experience-fragments.md). |
...
* È stato risolto un problema che impediva la corretta visualizzazione delle informazioni sulla regola del gruppo di destinatari nella finestra delle informazioni di [!UICONTROL Audiences Refinements]. (TGT-43917)
...
* Aggiornamento del token dell&#39;interfaccia utente [!DNL Target] abilitato per le organizzazioni abilitate con [account Business ID](https://helpx.adobe.com/enterprise/using/identity.html){target=_blank} e autenticazione basata su criteri (PBA). (TGT-42590)
...
* Le metriche Click-Track includeranno il payload di analisi nelle richieste API di recapito per le attività che utilizzano Analytics come origine del report (A4T) ed elaborano eventi sul lato client. (TNT-43073)

### Aggiornamento gruppi di destinatari [!DNL Target Standard] (28 marzo)
...
* È stato risolto un problema che impediva il corretto caricamento della pagina [!UICONTROL Administration] > [!UICONTROL Scene7 configuration]. (TGT-39918)
...
La versione di [!DNL Target] Standard/Premium del 12 novembre include miglioramenti di back-end, correzioni e modifiche. I report di [!UICONTROL Personalization Insights] saranno disponibili il 14 novembre.
...
* Migliorata la gestione dei tipi di pubblico eliminati in Adobe Audience Manager (AAM) a cui si fa riferimento nelle attività di Target. (TGT-23338)
   ...
   * Se un gruppo di destinatari è stato eliminato in AAM, viene visualizzata un&#39;icona di avviso sia nell&#39;elenco [!UICONTROL Audience] che nel selettore gruppi di destinatari. Un suggerimento nell&#39;interfaccia utente indica inoltre che il pubblico è stato eliminato in AAM.
...
* È stato risolto un problema che impediva agli utenti in determinate situazioni di creare un&#39;attività quando Adobe Analytics era selezionato come origine dei report nella pagina [!UICONTROL Administration]. Gli utenti visualizzavano il messaggio “Seleziona una suite di rapporti” anche se non veniva data la possibilità di selezionarla. (TGT-31968)
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p> 
     ...
     </ul> </p> <p>Per ulteriori informazioni, vedere <a href="/help/main/c-target/apply-reporting-audience-success-metric.md#concept_5F11149ACCA84FE79C7B9F766B6B0595" format="dita" scope="local"> Applicare un gruppo di destinatari di report a una metrica di successo </a>. </p> </td> 
  </tr> 
  <tr> 
...
<table id="table_8525ECC9B6D0435ABEF8C27F747B7A0C"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td colname="col2"> <p>Da oggi, Internet Explorer 11 è supportato dall'interfaccia di Target. </p> <p>Vedere <a href="https://experienceleague.adobe.com/docs/target-dev/developer/implementation/supported-browsers.html" format="dita" scope="local"> Browser Supportati </a>. </p> </td> 
  </tr> 
  <tr> 
...
* L&#39;aggiornamento della versione 48 di Chrome ha generato un problema di malfunzionamento del Compositore esperienza visivo su Chrome. Google sta lavorando alla soluzione. Per informazioni, fai riferimento a [https://code.google.com/p/chromium/issues/detail?id=582603](https://code.google.com/p/chromium/issues/detail?id=582603). Per aggirare questo problema:
   ...
   * Utilizza Firefox o Internet Explorer.
   * Abilita Enhanced Experience Composer, che può essere configurato dalla scheda **[!UICONTROL Administration]** > **[!UICONTROL Visual Experience Composer]**.
...
<table id="table_907A952F54084C2A9C61F10E2B7A7BFF"> 
 ...
 <tbody> 
  <tr> 
   ...
   <td colname="col2"> <p> Visualizza il tuo sito come si presenta su vari dispositivi mobili e schermi di diverse dimensioni. Imposta i punti di interruzione del sito reattivo e utilizzali nelle attività di ottimizzazione per assicurarti che vengano visualizzate al meglio su tutti i dispositivi usati dai visitatori. </p> <p>Per le esperienze reattive <a href="/help/main/c-experiences/c-visual-experience-composer/mobile-viewports.md#concept_8E45527C4ABC41D59AA3553BEDC76FA5" format="dita" scope="local">, vedere </a> finestre di visualizzazione per dispositivi mobili. </p> </td> 
  </tr> 
  <tr> 
...
| Funzionalità/Miglioramento | Descrizione |
|---|---|
| Consentire l&#39;inserimento e la modifica di JavaScript | È stata aggiunta la possibilità di modificare e inserire JavaScript personalizzato nell&#39;editor esperienze quando si sceglie **[!UICONTROL Edit HTML]** dal menu Azioni. |
