...
# Targeting automatico Domande frequenti e risoluzione dei problemi
...
* È consigliabile completare un’attività A/B tra le offerte e le posizioni che intendi utilizzare nel tuo [!UICONTROL Targeting automatico] attività per garantire che posizioni e offerte abbiano un impatto sull’obiettivo di ottimizzazione. Se un’attività A/B non riesce a dimostrare una differenza significativa, il [!UICONTROL Targeting automatico] probabilmente non riuscirà a generare incrementi.
...
### Adobe consigliato utilizzando [!UICONTROL Targeting automatico] con una suddivisione 90(Controllo)/10(Target) fino a quando i modelli non vengono generati?
...
+++Risposta Sono necessari quattro fattori per [!UICONTROL Targeting automatico] attività per generare l’incremento:
...
* Dopo che un’attività è in esecuzione, [!UICONTROL Targeting automatico] considera fino agli ultimi 45 giorni di dati serviti in modo casuale durante il tentativo di creazione di modelli (ad esempio, il traffico di controllo e alcuni dati forniti in modo casuale dall’algoritmo).
...
+++Risposta Puoi iniziare a esaminare i risultati [!UICONTROL Targeting automatico] verifica dopo aver creato almeno due esperienze con modelli (segno di spunta verde) per l’esperienza che presenta modelli generati.
...
+++L’Adobe di risposta non consiglia di modificare la metrica obiettivo a metà strada attraverso un’attività. Anche se è possibile modificare la metrica dell’obiettivo durante un’attività utilizzando l’interfaccia utente [!DNL Target], è sempre necessario avviare una nuova attività. Non forniamo alcuna garanzia su ciò che accade se modifichi la metrica dell’obiettivo in un’attività dopo l’esecuzione.
...
+++Risposta
[!DNL Target] crea un modello per esperienza, quindi rimuovere un’esperienza significa [!DNL Target] crea un modello in meno e non influisce sui modelli delle altre esperienze.
...
Quindi, assicurati che le offerte e le posizioni di attività facciano realmente la differenza nei tassi di risposta complessiva tramite un semplice test A/B non personalizzato. Calcola le dimensioni del campione in tempo per assicurarti che sia possibile visualizzare un incremento ragionevole ed esegui il test A/B per una durata fissa senza interruzioni né modifiche. Se i risultati di un test A/B mostrano un incremento statisticamente significativo su una o più esperienze, è probabile che un’attività personalizzata funzioni. Naturalmente, la personalizzazione può funzionare anche se non ci sono differenze nei tassi di risposta complessiva delle esperienze. In genere, il problema deriva dal fatto che le offerte e le posizioni non hanno un impatto sufficiente sull’obiettivo di ottimizzazione da rilevare con significatività statistica.
