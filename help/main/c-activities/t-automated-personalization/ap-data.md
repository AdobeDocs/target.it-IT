...
## Personalizzato [!DNL Target] categorie di attributi

La tabella seguente mostra i dati forniti dal cliente e raccolti da [!UICONTROL Automated Personalization] e [!UICONTROL Targeting automatico] attività. Questi dati vengono raccolti solo se forniti dall&#39;utente. Nomi di attributi specifici e valori di esempio sono specifici per la configurazione del sistema.
...
| [!DNL Target] profilo | PRO | Gli attributi di profilo personalizzati vengono caricati direttamente in [!DNL Target] profilo tramite API o parametro di pagina e [!DNL Target] script di profilo. | Personalizzato - Profilo visitatore - [nome attributo] |
| Attributi del cliente | CRS | Attributi del cliente caricati in [!DNL Target] profilo tramite [[!DNL Adobe Experience Cloud Customer Attributes Service]](https://experienceleague.adobe.com/docs/core-services/interface/services/customer-attributes/attributes.html){target=_blank}. | Personalizzato - Profilo visitatore - [nome attributo] |
...
| [!DNL Adobe Experience Cloud] pubblici condivisi | AAM | Tutti i tipi di pubblico condivisi con [!DNL Target] da altri [!DNL Adobe Experience Cloud] soluzioni (ad esempio [!DNL Adobe Audience Manager] e [!DNL Adobe Analytics], tramite [[!DNL Experience Cloud Audience Library]](https://experienceleague.adobe.com/docs/core-services/interface/services/audiences/audience-library.html){target=_blank}). | Personalizzato - Pubblico Experience Cloud - [Nome pubblico] |
| [!DNL Adobe Experience Platform Real-time CDP] audiences | UPS | Pubblico di Platform Real-time CDP condiviso con [!DNL Target] tramite [!UICONTROL Destinazioni]. |  |
| [!DNL Adobe Experience Platform Real-time CDP] attributi | AEP | Attributi di Platform Real-time CDP condivisi con [!DNL Target] tramite [!UICONTROL Destinazioni]. |  |

## Blocco delle funzioni da [!DNL Target] algoritmi di apprendimento automatico

Le funzionalità possono essere bloccate [!DNL Target] algoritmi di apprendimento automatico, che ne impediscono l’utilizzo in qualsiasi [!UICONTROL Automated Personalization] o [!UICONTROL Targeting automatico] modello o attività.

Per ulteriori informazioni, consulta [Panoramica di Models API (Inserisce nell&#39;elenco Bloccati di)](https://experienceleague.adobe.com/docs/target-dev/developer/api/models-api/models-api.html){target=_blank} nel *[!DNL Adobe Target]Guida per gli sviluppatori*.
...
| Mobile - OS - OS X ([!DNL Android], [!DNL Windows]e così via) | Specifica se l’utente ha utilizzato un sistema OSX (o [!DNL Android], [!DNL Windows]e così via) per accedere all’attività. | 0 = False, 1 = True | MOB_targeting.mobile.os[SO]<br>Ad esempio, MOB_targeting.mobile.osOSx, MOB_targeting.mobile.osWindows, MOB_targeting.mobile.osAndroid, MOB_targeting.mobile.osLinux |
...
| Browser - Hour of Week | L’ora della settimana in cui il visitatore ha effettuato l’accesso all’attività. | 0 - 168<br>(Domenica mezzanotte è 0) | ENV_WeekHour |
...
| Browser - Weekday/Weekend | Lo stato lavorativo del giorno in cui il visitatore accede all’attività (fine settimana, orario lavorativo, orario non lavorativo infrasettimanale). | Il sabato e la domenica sono weekend<br>Dal lunedì al venerdì dalle 09:00 alle 18:00 orario di lavoro<br>Dal lunedì al venerdì dalle 18:00 alle 09:00 è il giorno feriale libero | ENV_UserHourType |
...
| Sistema operativo | Il sistema operativo del dispositivo utilizzato dal visitatore per accedere all’attività. | [!DNL Mac OS]<br>[!DNL Windows]<br>[!DNL Linux]<br>Cerca bot<br>Sistema operativo sconosciuto | ENV_OperatingSystem |
...
| Visitor Profile - Page Views in Current Visit | Specifica il numero di visualizzazioni di pagina durante la visita/sessione corrente fino a quando il visitatore accede all’attività. Più precisamente, il numero di impression. Le impression non sono visualizzazioni di pagina reali, ma piuttosto quante volte la richiesta è stata ricevuta da [!DNL Target]. [!DNL Target] non è in grado di distinguere tra timeout o altri motivi per cui l’utente non ha ricevuto o visualizzato il contenuto. | Doppio (solo numero intero positivo). | SES_SESSION_POSITION |
| Visitor Profile - Start of Current Visit | Specifica l’ora in cui viene effettuata la visita/sessione corrente con [!DNL Target] ha iniziato. La visita con [!DNL Target] può essere avviata senza accedere a un’attività. È sufficiente una chiamata a qualsiasi [!DNL Target] richiesta. L’accesso all’attività e l’acquisizione dell’istantanea potrebbero richiedere del tempo. | Doppio, millisecondi | SES_SESSION_START |
| Visitor Profile - Start of Most Recent Visit | Specifica l’ora dell’ultima visita/sessione con [!DNL Target] ha iniziato. Questo attributo viene aggiornato alla scadenza della sessione.<br>Se si tratta della prima sessione del visitatore, viene generato `LAST_SESSION_START = 0.` | Doppio, millisecondi | SES_LAST_SESSION_START |
...
| Visitor Profile - Time in Visit Before Enter Activity | Specifica la differenza tra l’ultima interazione con [!DNL Target] e quando è iniziata la visita corrente. Questo attributo può essere considerato come durata della visita/sessione fino al momento in cui l’utente accede all’attività e viene acquisita l’istantanea.<br>Valori negativi si verificano quando l’inizio della sessione e l’ultima ora di aggiornamento vengono attivati dallo stesso [!DNL Target] chiamare. I valori negativi devono essere considerati 0 (zero). | Doppio, millisecondi | SES_SESSION_TIME |
